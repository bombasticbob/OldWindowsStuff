...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow, BOOL ForceExternalLibrary);
...
//
   ...
   if(SpawnThreadX((LPTHREADPROC)CommandThread, NULL, THREADSTYLE_DEFAULT,
                    NULL,COMMAND_STACK_SIZE)==-1 ||
      SpawnThread((LPTHREADPROC)CopyThread, NULL, THREADSTYLE_DEFAULT)==-1 ||
      SpawnThread((LPTHREADPROC)FormatThread, NULL, THREADSTYLE_DEFAULT)==-1 ||
      SpawnThreadX((LPTHREADPROC)BatchThread, NULL, THREADSTYLE_DEFAULT,
                    NULL,COMMAND_STACK_SIZE)==-1 ||
      SpawnThread((LPTHREADPROC)FlashThread, NULL, THREADSTYLE_DEFAULT)==-1)
   {
...
BOOL FAR PASCAL EXPORT AuthCodeDialog(HWND hDlg, UINT uiMsg,
...
char tbuf[256];
   ...
   *tbuf = 0;
   ...
   if(!(lpCmdHist = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                          CMDHIST_SIZE)))
   {
      return(FALSE);
   }
...
//               MySetFileAttr(szFontName, _A_RDONLY);
   ...
   hbmpToolBar = LoadBitmap((HINSTANCE)hInstance, "TOOLBAR");
   ...
   lpScreen = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                    (sizeof(pScreen[0][0]) + sizeof(pAttrib[0][0]))
                                     * SCREEN_COLS * SCREEN_LINES);

   if(!lpScreen) return(FALSE);

   pScreen = (SCREEN_BASED_PLINE)lpScreen;
   pAttrib = (SCREEN_BASED_PLINE)
             (lpScreen + sizeof(pScreen[0][0]) * SCREEN_COLS * SCREEN_LINES);
...
HFONT hOldFont;
   ...
   if(!TMFlag)   /* if we haven't gotten the TEXT METRICS yet... */
   {
      ...
      {
         hOldFont = (HFONT)SelectObject(hDC, DEFAULT_FONT);
      }
...
HPEN hOldPen;
   ...
   hOldPen = (HPEN)SelectObject(hdcBar, hpenDarkGrey); // select dark grey pen
...
int iOldStretchMode;
   ...
   hOldPen = (HPEN)SelectObject(hdcBar, hpenBlack);        // select black pen
...
//      }
   ...
   else
   {
    ...
    static const short CODE_BASED pDither[]=
      ...
      hbmpOld = (HBITMAP)SelectObject(hDC2, hbmpCompat);
      ...
      hbrOld = (HBRUSH)SelectObject(hdcBar, hbrPattern);
...
//DWORD dwDialogUnits = GetDialogBaseUnits();
   ...
   hOldBitmap = (HBITMAP)SelectObject(hdcBMP, hbmpToolBar);
...
//   dwDialogUnits = GetDialogBaseUnits();
   ...
   if(ShowToolBar)
   {
      ...
      hOldBarBitmap = (HBITMAP)SelectObject(hdcBar, hBarBitmap);
      ...
      hOldBitmap = (HBITMAP)SelectObject(hDC2, hbmpToolBar);
...
//                                               // and execute it first
      ...
      case IDM_COPY_ALL:
         ...
         for(lp2 = lp1, i=0; i<=maxlines && i<SCREEN_LINES; i++)
         {
            ...
            {
               if(pScreen[i][k])
                break;  /* finds last non-null character */
            }

            for(j=0; j<=k; j++)
            {
               if(*lp2 = pScreen[i][j]) lp2++;
               else  *(lp2++) = ' ';
            }
            *(lp2++) = '\r';
            *(lp2++) = '\n';
      ...
      case IDM_PASTE_MENU:
         ...
         {
            if(SpawnThread((LPTHREADPROC)MemoryBatchFileThreadProc, h, THREADSTYLE_DEFAULT))
            {
               CloseClipboard();
...
//         start_line = curline;
      ...
      else if(ch=='\x7f') /* DELETE!! like bksp except cursor doesn't move */
      {
         ...
         if(cmd_buf_len > (iCMDBUF + 1))
         {
            ...
            _hmemmove(pScreen[curline] + curcol,
                      pScreen[curline] + curcol + 1,
                      cmd_buf_len - iCMDBUF - 1);

            _hmemmove(pAttrib[curline] + curcol,
                      pAttrib[curline] + curcol + 1,
                      cmd_buf_len - iCMDBUF - 1);
         }

         cmd_buf[--cmd_buf_len] = 0;

         pScreen[end_line][end_col] = 0;
         ...
         pScreen[end_line][end_col] = 0;  // the *NEW* end must be cleared
      ...
      else if(ch=='\x8')  /* backspace!! */
      {
         if(iCMDBUF > 0)  // can't backspace at beginning of line
         {
            if(cmd_buf_len > iCMDBUF)
            {
               ...
               _hmemmove(pScreen[curline] + curcol - 1,
                         pScreen[curline] + curcol,
                         cmd_buf_len - iCMDBUF);

               _hmemmove(pAttrib[curline] + curcol - 1,
                         pAttrib[curline] + curcol,
                         cmd_buf_len - iCMDBUF);
            }
...
static COLORREF back_colors[8]={
   ...
   for(i=vpos, i0=0; i0<=lines_per_screen; i++,i0++)
   {
      ...
      for(j=hpos, j0=0; j0<=columns_per_screen; j++,j0++)
      {
         ...
         if(i>=SCREEN_LINES || j>=SCREEN_COLS || (k0 = pScreen[i][j])==0)
         {
            temp_buf[k++] = ' ';
         }
         else
...
long l;
   ...
   switch(msg)
   {
      ...
      case WM_COMMAND:
         ...
         if((WORD)wParam == IDOK)
         {
          LPSTR lp1;
            ...
            lp1 = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                        (sizeof(pScreen[0][0]) + sizeof(pAttrib[0][0]))
                                        * SCREEN_COLS * l);
            ...
            else
            {
               ...
               pScreen = (char (__based(lpScreen) *)[80])lpScreen;
               pAttrib = (char (__based(lpScreen) *)[80])
                          (lpScreen + sizeof(pScreen[0][0])
                          * SCREEN_COLS * SCREEN_LINES);
      ...
      case WM_DRAWITEM:
         ...
         hbmpOld2 = (HBITMAP)SelectObject(hDC2, hbmpBtn);
            ...
            case IDC_DIRLIST:

               {
                ...
                HDC hDC = GetDC(GetDlgItem(hDlg, IDC_DIRLIST));
                  ...
                  if(hCurFont) hOldFont = (HFONT)SelectObject(hDC, hCurFont);
                  else         hOldFont = NULL;
...
static WORD bHasCD = 0, bHasCDChecked = 0;
   ...
   else if(wDriveType == DRIVE_REMOVABLE)
   {
      ...
      GetDriveParms(GetPhysDriveFromLogical(iDrive + 1),
                    &wDriveType,
                    NULL, NULL, NULL, NULL);
...
TEXTMETRIC tm;
   ...
   *tbuf = 0;
   ...
   hOldFont = (HFONT)SelectObject(lpDrawItemStruct->hDC, hCurFont);
...
//      hBitmap = GetDriveBitmap(toupper(*tbuf) - 'A' + 1);
      ...
      hOldBitmap2 = (HBITMAP)SelectObject(hCompatDC2, hBitmap);
...
//      CurDirIndex = SendDlgItemMessage(hDlg, IDC_DIRLIST, LB_GETCURSEL, 0, 0);
      ...
      if(lpDrawItemStruct->itemID < CurDirIndex)
      {
         ...
         hOldBitmap = (HBITMAP)SelectObject(hCompatDC, hbmpOpenFolder);
      }
      else if(lpDrawItemStruct->itemID == CurDirIndex)
      {
         GetObject(hbmpGrayFolder, sizeof(bmp), (LPSTR)&bmp);
         hOldBitmap = (HBITMAP)SelectObject(hCompatDC, hbmpGrayFolder);
      }
      else
      {
         GetObject(hbmpShutFolder, sizeof(bmp), (LPSTR)&bmp);
         hOldBitmap = (HBITMAP)SelectObject(hCompatDC, hbmpShutFolder);
      }
...
//               break;
            ...
            case IDYES:
               ...
               if(SpawnThread((LPTHREADPROC)MemoryBatchFileThreadProc, h1,
                              THREADSTYLE_DEFAULT))
               {
...
char temp_buf[128];
   ...
   switch(wType)
   {
      ...
      case XTYP_ADVSTART:
         ...
         {
            lpHotLinkItems = (LPSTR)GlobalAllocPtr(GHND, 0x1000);
         ...
         else
         {
            ...
            if(!*lp1)  // item not found - add to the list!
            {
               ...
               if(w >= GlobalSizePtr(lpHotLinkItems))
               {
                  ...
                  lp2 = (LPSTR)GlobalReAllocPtr(lpHotLinkItems,
                                                (w + 0x1fff) & 0xf000L,
                                                GMEM_MOVEABLE);

                  if(!lp2)
                    return((HDDEDATA)0);  // not enough memory!
...
//         if(wFmt==CF_OEMTEXT) OemToAnsi(temp_buf, temp_buf);
      ...
      case XTYP_POKE:
         ...
         else
         {
            ...
            {
               lp1 = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, w+1);
