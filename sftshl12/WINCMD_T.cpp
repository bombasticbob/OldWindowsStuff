...
LPSTR lpTemp;
   ...
   while(!LoopDispatch())
   {
      ...
      {                      /* there's a command waiting to execute! */
         ...
         lpTemp = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE, lstrlen(lpCurCmd)+1);
...
BOOL bFlag;
   ...
   lp1 = (LPSTR)GlobalLock(hParms);
   if(!lp1)  return(1);

   lpProg = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE, lstrlen(lp1) + 1);
   if(!lpProg)
   {
      GlobalUnlock(hParms);
      return(2);
   ...
   if(!bFlag)
   {
      ...
      ExecuteBatchFile(NULL,                       // execute it, NOW!  Returns
                       (DOSRUNPARMBLOCK *)lpProg); // immediately, and batch file
...
extern HWND hdlgQueue;  // declared in wincmd.c
   ...
   lpF2 = (LPFORMAT_REQUEST)GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
