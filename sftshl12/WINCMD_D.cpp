...
char tbuf[64];
   ...
   {
      lpC = (LPCURDIR)GetDosCurDirEntry(wDrive + 1);   // 'wDrive' is 1-based for this
      ...
      while(lpC && lpC->wFlags & CURDIR_SUBST)  // this is a SUBST'ed drive
      {
         ...
         lpC = (LPCURDIR)GetDosCurDirEntry(wDrive + 1);
      }
...
//      }
   ...
   lpC = (LPCURDIR)GetDosCurDirEntry(wDrive + 1);  // 'GetDosCurDirEntry()' is 1-based
...
DWORD dwVer, dwAddr;
   ...
   do
   {
      ...
      {
         lpTDB = (TDB FAR *)GlobalLock(te.hTask);
...
BOOL fOddEven;
   ...
   if(!*lpFATBuf)
   {
      *lpFATBuf = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
