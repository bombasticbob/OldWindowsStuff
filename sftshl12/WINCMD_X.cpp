...
LPSTR lpShell;
   ...
   if(!hRexxAPI)    /* have not found the REXX API yet? */
   {
      ...
      lpRxFunctionRegister((LPBYTE)szMessageBox,(LPBYTE)pWRXTRNL,(LPBYTE)pWrXtrnlFn,
                            RXFUNC_DYNALINK);
      lpRxFunctionRegister((LPBYTE)szCancelBox,(LPBYTE)pWRXTRNL,(LPBYTE)pWrXtrnlFn,
                            RXFUNC_DYNALINK);
      lpRxFunctionRegister((LPBYTE)szQuestionBox,(LPBYTE)pWRXTRNL,(LPBYTE)pWrXtrnlFn,
                            RXFUNC_DYNALINK);
      lpRxFunctionRegister((LPBYTE)szPromptBox,(LPBYTE)pWRXTRNL,(LPBYTE)pWrXtrnlFn,
                            RXFUNC_DYNALINK);
      lpRxFunctionRegister((LPBYTE)szChoiceBox,(LPBYTE)pWRXTRNL,(LPBYTE)pWrXtrnlFn,
                            RXFUNC_DYNALINK);
    
      lpRxFunctionRegister((LPBYTE)szGetProfile,(LPBYTE)pWRXTRNL,(LPBYTE)pRxWinUtil,
                            RXFUNC_DYNALINK);
      lpRxFunctionRegister((LPBYTE)szSetProfile,(LPBYTE)pWRXTRNL,(LPBYTE)pRxWinUtil,
                            RXFUNC_DYNALINK);
...
//   }
   ...
   lprxSysExit[0].sysexit_name = (LPBYTE)pRxHalt;  /* perform 'yields' */
   lprxSysExit[0].sysexit_code = RXHLT;

   lprxSysExit[1].sysexit_name = (LPBYTE)pRxSio;   /* trap SYSTEM i/o */
   lprxSysExit[1].sysexit_code = RXSIO;

   lprxSysExit[2].sysexit_name = (LPBYTE)pRxFnc;   /* trap EXTERNAL FUNCTIONS */
   lprxSysExit[2].sysexit_code = RXFNC;

   lprxSysExit[3].sysexit_name = (LPBYTE)pRxCmd;   /* trap WINCMD commands */
   ...
   if(IsCommand)  /* the 'lpArgs' string points directly to a command */
   {
      ...
      if(!*lpArgs)
      {
         lpShell = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE, sizeof(shell_prog)+1);
         if(!lpShell)
         {
            wRval = TRUE;
         }
         else
         {
            ...
            lprxsInStore[0].strptr = (BYTE FAR *)lpShell;
            lprxsInStore[0].strlength = lstrlen(lpShell);
         }
      }
      else
      {
         lprxsInStore[0].strptr = (BYTE FAR *)lpArgs;
      ...
      if(wRval ||
         lpREXXSAA(0, lprxsArg, (LPBYTE)pWINCMD, lprxsInStore, (LPBYTE)pWINCMD,
                   RXCOMMAND, lprxSysExit, &rcode, &rxsReturn))
      {
         if(wRval || !IsHalt)
         {
   ...
   {              /* and optionally arguments 'attached' to the thing.    */
      ...
      while(*lp1)
      {
         ...
         lprxsArg[j].strptr = (BYTE FAR *)lp2;
      ...
      if(lpREXXSAA(j, lprxsArg, (LPBYTE)lpArgs, NULL, (LPBYTE)pWINCMD,
                   RXCOMMAND, lprxSysExit, &rcode, &rxsReturn))
      {
         if(!IsHalt)
         {
         ...
         switch(i)
         {
            case 0:    /* CharOut */
               ...
               for(w=0, j=0; j<(WORD)ArgCount; j++)
               {
                  if(Args[j].strlength)
                  {
                     PrintBuffer((LPSTR)Args[j].strptr, (WORD)Args[j].strlength);
                     w += (WORD)Args[j].strlength;
                  }
               }

               lprxsReturn->strptr = (BYTE FAR *)GlobalAllocPtr(GMEM_MOVEABLE, 16);
               if(lprxsReturn->strptr)
               {
                  lprxsReturn->strlength =
                         wsprintf((LPSTR)lprxsReturn->strptr, "%d", w);
               }
   ...
   if(wExitFunc==RXSIO)  /* then we're in the RIGHT PLACE! */
   {
      switch(wExitSubFunc)
      {
         ...
         case RXSIOSAY:   /* output to terminal */
            ...
            PrintBuffer((LPSTR)lprxS->strptr, (WORD)lprxS->strlength);
         ...
         case RXSIOTRD:   /* get input from terminal */
            ...
            if(!lprxS->strptr)
            {
               ...
               lprxS->strptr = (BYTE FAR *)GlobalAllocPtr(GMEM_MOVEABLE, lprxS->strlength);
               if(!lprxS->strptr)
               {
            ...
            {
               lstrcpy((LPSTR)lprxS->strptr, work_buf);
               lprxS->strlength = lstrlen(work_buf);
            }
...
BOOL rval;
   ...
   if(wExitFunc==RXCMD && wExitSubFunc==RXCMDHST)
   {
      if((WORD)lpP->rxcmd_addressl!=(WORD)lstrlen(pWINCMD) ||
         _fstrnicmp((LPSTR)lpP->rxcmd_address, pWINCMD, lpP->rxcmd_addressl)!=0)
      {
         return(1);  /* this environment did NOT process the thing! */
      }
         
      lp1 = (LPSTR)GlobalAllocPtr(GMEM_MOVEABLE, lpP->rxcmd_command.strlength + 1);
      if(!lp1)
      {
...
//         {
            lpP->rxcmd_retc.strptr = (BYTE FAR *)GlobalAllocPtr(GMEM_MOVEABLE, 1);
//         }
...
WORD i, j, wRC;
   ...
   for(i=0; i<wCount; i++)
   {
      scb.scbname      = (BYTE FAR *)lpArgs->lpNames[i];
      ...
      if(!RxSubComRegistrationOK(wRC, (LPSTR)scb.scbname))
      {
         for(j=i; j>0; j--)     /* free all of the other assigned exits */
         {
            lpRxExitDrop((LPBYTE)lpArgs->lpNames[j - 1], NULL);
         }
