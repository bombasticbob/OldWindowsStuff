...
WORD w;
   ...
   lpPathArray = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
...
int _getdrive(void)
{
   ...
   if(FALSE) return(0);  /* will never happen... */
...
char FAR *tbuf=NULL;
   ...
   tbuf = GlobalAllocPtr(GPTR, 2 * MAX_PATH);
   if(!tbuf)
...
WORD wClusterSize, wFreeClusters, wSectorSize, wTotalClusters;
   ...
   if(wClusterSize == 0xffff)  // this is a sign of an error...
   {
      ...
      {
       LPCURDIR lpC = GetDosCurDirEntry(wDrive);  // check for SUBST drive
...
static struct {
   ...
   *lpDest = 0;                   /* initially assign empty string to dest */
   ...
   if(GetDriveType(i - 1)<=1)   // drive does not exist
   {
      ...
      {
         lpC = GetDosCurDirEntry(i);  // check for SUBST drive
   ...
   else
   {
     ...
     *lpDest = 0;
     ...
     if(uVersion.ver.dos_major>=4)          /* DOS version 4.0 or above... */
     {
        ...
        lpDosMem = MAKELP(LOWORD(dwDosMem), 0);
...
HCURSOR hCursor;
   ...
   lpTree = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
   ...
   {
      lpT2 = GlobalReAllocPtr(lpTree, (wNext + 1) * sizeof(TREE),
...
//            strcmp(pFF->name,".")!=0 && strcmp(pFF->name,"..")!=0)
         {
            ...
            if((*lpwNext + 1)>=*lpwTreeSize)
            {
               ...
               lpTree = GlobalReAllocPtr(*lplpTree,
...
//               ctrl_break = FALSE;    // eat the control break
         ...
         if((attrib1 & ff.attrib) == attrib1)
         {
            ...
            {
               lpDI0 = GlobalReAllocPtr(lpDirInfo, cur_size +
                                                   64L * sizeof(DIR_INFO),
                                        GMEM_MOVEABLE | GMEM_ZEROINIT);
               if(!lpDI0)
               {
...
char FAR *tbuf=NULL, FAR *tbuf2=NULL;
   ...
   tbuf = GlobalAllocPtr(GPTR, MAX_PATH * 4);
   if(!tbuf)
     return;
...
static struct DOSERROR save_error, pErr[32];
...
void SaveDosErrorCodes(void)
{
   ...
   _asm
   {
      ...
      mov BYTE PTR save_error.class, bh
...
static DOSDPL dpl;
   ...
   dpl._bx = (save_error.class << 8) | (save_error.action & 0xff);
...
LPSTR lp1, lp2;
   ...
   lp1 = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, 16);
...
BOOL last_was_space=FALSE, OldBatchMode;
   ...
   else      // MEMORY BATCH FILE:  'lpParmBlock' points to the actual code
   {
    LPSTR lpCode = (LPSTR)lpParmBlock;
      ...
      lpNewBatch->lpArgList = lpNewBatch->buf;
      lpNewBatch->nArgs = 0;        // zero arguments

      lstrcpy(lpNewBatch->buf, lpCode);
...
char tbuf[64];
   ...
   {
      lpNewEnv = GlobalAllocPtr(GPTR,
                         GlobalSize(GlobalHandle(lpParmBlock->wEnvSeg)) + 4);
      if(lpNewEnv)
      {
       LPSTR FAR *lplpEnv;
         ...
         for(dw2=0, lp1 = MAKELP(lpParmBlock->wEnvSeg, 0); *lp1;
         ...
         for(dw1=0, lp1 = MAKELP(lpParmBlock->wEnvSeg, 0); *lp1;
         ...
         _lqsort(lplpEnv, dw2, sizeof(*lplpEnv), EnvPointerCompare);
...
//                              (DWORD)STD_ERROR_HANDLE);
      ...
      {
         lpBuf = GlobalAllocPtr(GPTR, lstrlen(lpProgName)
                                      + lstrlen(tbuf)
      ...
      {
         lpBuf = GlobalAllocPtr(GPTR, lstrlen(tbuf)
                                      + lpParmBlock->lpCmdLine[0] + 2);
      }
      ...
      else
      {
         ...
         lpProgName = szHelperProg;    // 'helper' app for 32bit and WINOLDAP
      }
      ...
      if(!lpNewEnv)
      {
         lpEnv0 = MAKELP(lpParmBlock->wEnvSeg,0);
      }
      else
      {
         lpEnv0 = lpNewEnv;
