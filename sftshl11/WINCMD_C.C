...
LPSTR lpTempBuf;
   ...
   lpTempBuf = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, 512);
   if(!lpTempBuf)
   {
...
LPDIR_INFO lpDirInfo = NULL, lpDI;
   ...
   if(!(lpParseInfo = CMDLineParse(lpArgs)) ||
      !(lpTemp = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                MAX_PATH * 2)))
   {
...
WORD w, wCount;
   ...
   if(!(hTask = (HTASK)MyValue(lpArgs)))
   {
     ...
     /** It's not a valid handle, so look for a module name that matches **/
      ...
      wCount = 0;
      lphTask = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
      ...
      case CMDDDE_POKE:
         ...
         if(wOption==CMDDDE_REQUEST)  /* 'hData' contains result! */
         {
            if(hData && !_fstricmp(GetEnvString(pDDERESULT),pOK))
            {
               ...
               if(w)
               {
                  lp3 = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, w+1);
                  if(!lp3)
                  {
                     PrintString(pNOTENOUGHMEMORY);
                     SetEnvString(pDDERESULT,pERROR);
...
char tbuf[16];
   ...
   if(!(lpDirPath = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                sizeof(work_buf))))
   {
      PrintString(pNOTENOUGHMEMORY);
      return(TRUE);
...
#define DIRPATH_SIZE 512      /* size of 'lpDirPath' */
   ...
   {
      lpTempBuf = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
   ...
   lpDirPath = GlobalAllocPtr(GMEM_MOVEABLE|GMEM_ZEROINIT,
         ...
         else
         {
            ...
            if((IsChicago || IsNT) && *szLFNBuf)   // LFN found on file name!
            {
               ...
               if(redirect_output==HFILE_ERROR)
               {
                  ...
                  do
                  {
                     if(lstrlen(lp1) < (79 - (lp2 - work_buf)))
                     {
                        lstrcat(work_buf, lp1);
                        break;
                     }
                     ...
                     if(*lp1)
                     {
                        ...
                        wsprintf(work_buf + lstrlen(work_buf),
                                 "\x1b[K\x1b[%dC", (lp2 - work_buf));
                     }
...
{
LPPARSE_INFO lpParseInfo;
WORD i, j, k, wDrive, wDriveType, wFormatSize, wTracks, wSectors, wHeads,
     wSwitchFlag, wBootDrive;
LPSTR lpSwitch, lp1;
...
REALMODECALL rmc;
   ...
   if(*label_buf>' ')           /* there is currently a label! */
   {
      ...
      for(lp1=fcb.name, lp2=label_buf, w=0; w<11; w++)
      {
         if(*lp2>' ')
         {
            *lp1++ = *lp2++;
...
BOOL rval;
   ...
   if(*lp1)
   {
      ...
      if(*lp2)
      {
         ...
         {
            lp3 = GlobalAllocPtr(GMEM_MOVEABLE, lstrlen(lp2) + 2);
            if(!lp3)
            {
               PrintString(pNOTENOUGHMEMORY);
               GlobalFreePtr(lpArgs);
            ...
            else
            {
               ...
               else
               {
                  ...
                  if(*lp1 == '\\' && lp1[1] == '\\')
                  {
                     ...
                     if(!*lp1)  // just in case
                        lp1 = MAKELP(wMySel, 0);  // restore it if none found
                     else // if(*lp1 == '\\')
               ...
               lp1 = MAKELP(wMySel, 0);  // restore the string pointer
    ...
    static const char CODE_BASED szDevices[][5]={
      ...
      if(!lpItem2)  // no argument after "USE"
      {
         ...
         for(i1=0; i1 < N_DIMENSIONS(szDevices); i1++)
         {
            ...
            UINT uiRval = lpWNetGetConnection(szDevices[i1], tbuf, &cbSize);
...
struct _find_t ff;
   ...
   lpSrcPath = GlobalAllocPtr(GMEM_MOVEABLE, sizeof(work_buf) +
...
//                        (LPARAM)(LPSTR)&dbv);
         ...
         lpC->pathname[0] = wDrive + 'A' - 1;  /* the drive letter desig. */
         lpC->pathname[1] = ':';
         lpC->pathname[2] = '\\';
         lpC->pathname[3] = 0;
...
/*         SendMessage(HWND_BROADCAST, WM_FILESYSCHANGE, 0, 0); */
      ...
      else   /* 2nd arg must be a valid path */
      {
         ...
         if(!(lpC2 = GetDosCurDirEntry(*work_buf - 'A' + 1)))
         {
            MyFreeSelector(SELECTOROF(lpC));
...
WORD wLevelArray[64];
   ...
   lpDirPath = GlobalAllocPtr(GMEM_MOVEABLE, sizeof(work_buf) +
                                           lstrlen(lpPath) + lstrlen(lpDest));
   if(!lpDirPath)
   {
      PrintString(pNOTENOUGHMEMORY);
