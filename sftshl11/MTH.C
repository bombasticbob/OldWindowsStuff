...
static void (FAR PASCAL *CompactProc)(void) = {
                                               (void (FAR PASCAL *)())NULL};
static void (FAR PASCAL *AppMessageProc)(LPMSG lpmsg)= {
                                               (void (FAR PASCAL *)())NULL};
...
DWORD dwAccelEntry;
   ...
   {
      Accelerators = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT
                                    | GMEM_DDESHARE,
...
WORD w;
   ...
   {
      MDIAcceleratorsClient = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
...
WORD w;
   ...
   {
      DialogWindowList = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                             sizeof(HWND) * MAX_MDIACCEL);
...
WFPPROC _SpawnThread(LPTHREADPROC lpProcName, HANDLE hArgs, WORD flags)
{
   ...
   return(SpawnThreadX((LPSTR)lpProcName, hArgs, flags, NULL, THREAD_STACK));
}


WFPPROC _SpawnThreadX(LPSTR lpProcName, HANDLE hArgs, WORD flags,
...
//// ** PAGE LOCK STACK ** 7/16/98
   ...
   if(!NewThread.hThreadID ||              /* error creating thread? */
      !(lpIT = GlobalLock(NewThread.hThreadID)))
   {
   ...
   else if(!(lpDot = _fstrchr(lpProcName,'.')))
   {
            /* no '.' in proc name - use current module! */

      threadproc = GetProcAddress(NULL,lpProcName);
   ...
   else
   {
      ...
      *lpDot = 0;
      threadproc = GetProcAddress(GetModuleHandle(lpProcName),
                                  lpDot + 1);
   ...
   if(threadproc && (LPSTR)threadproc!=lpProcName) /* it's a string! */
   {
      if(!lpDot)
      {
       _segment old_seg, new_seg;
        ...
        new_seg = HIWORD(GlobalLock(_hInst));
...
HANDLE FAR PASCAL _GetInstanceTask(HINSTANCE hInst)
{
TASKENTRY te;
...
HFPPROC FAR PASCAL _GetTaskInstance(HTASK hTask)
{
TASKENTRY te;
   ...
   else if(GetInputState())
   {
      ...
      if(wYieldFlag!=1)            /* not called from MthreadGetMessage() */
      {
         if(wInternalThreadIndex) /* if I'm being killed, don't yield! */
         {
            ...
            lpIT = GlobalLock(lpThreads[wInternalThreadIndex]);
...
//                                        PM_NOYIELD | PM_NOREMOVE));
   ...
   {
      lpThreads = (HANDLE FAR *)GlobalLock(hInternalThreadList);  /* yahoo! get the list */
      lpIT = GlobalLock(lpThreads[wInternalThreadIndex]); /* CURRENT thread */
   }
...
//            USE_32_BIT_OPERAND  _asm pushf
   ...
   {           /* there's a new thread in town - fix it up! */
    ...
    static HANDLE FAR *lpThreads2;
      ...
      lpIT2 = GlobalLock(lpThreads2[0]);                /* 'ROOT' thread */
      ...
      if(!lpIT && (wYieldFlag!=2 || wInternalThreadCount<=1))
      {
         ...
         if(!wOldIndex && !InSendMessage() && wYieldFlag == 1)
         {
          BOOL bOldIdleMessage = mtIdleMessage;
            ...
            lpIT = GlobalLock(lpThreads[0]); /* the 'root' thread (msg loop) */
...
HANDLE hTemp;
   ...
   lpIT = GlobalLock(hThreadID);  /* get pointer to thread information */
   ...
   {
      _fmemcpy(lpThreads + i, lpThreads + i + 1,
...
MSG m;
   ...
   for(i=0; i<MAX_SEMAPHORECALLBACKS; i++)
   {
      ...
      {
         was_callback=TRUE;
         PostAppMessage(hsemproc_task[i], WM_SEMAPHORECALLBACK,
...
BOOL rval = 0;
   ...
   hInst = GetCurrentInstance();
   lpOwnerDS = GlobalLock(hInst);       /* lock the owner's data segment! */
...
HANDLE FAR *lpThreads;
   ...
   for(i=0; i<wInternalThreadCount; i++)
   {
      lpIT = GlobalLock(lpThreads[i]);
...
HANDLE (FAR PASCAL * PASCAL GetInstanceTask)(HANDLE)                             = _GetInstanceTask;
HANDLE (FAR PASCAL * PASCAL GetTaskInstance)(HANDLE)                             = _GetTaskInstance;
...
WORD (FPPROC * PASCAL SpawnThreadX)(LPSTR, HANDLE, WORD, LPSTR, WORD)            = _SpawnThreadX;
