...
LPSTR lp1, lp2, lp3, lp4, drive, dir, name, ext, lpSwitch;
   ...
   lp1 = GlobalAllocPtr(GMEM_FIXED | GMEM_ZEROINIT,
   ...
   {
    LPCURDIR lpC = GetDosCurDirEntry(toupper(*drive) - 'A' + 1);
...
char FAR *pBuf=NULL, FAR *pBuf2=NULL;
   ...
   pBuf = GlobalAllocPtr(GPTR, MAX_PATH * 4);
   if(!pBuf)
   {
     GlobalCompact(2 * MAX_PATH);
     
     pBuf = GlobalAllocPtr(GPTR, 4 * MAX_PATH);
   }
...
char FAR *tbuf = NULL;
   ...
   if(IsNT && !IsChicago)
   {
      tbuf = GlobalAllocPtr(GHND, MAX_PATH);
      if(!tbuf)
         return(-1);
...
LPWIN32_FIND_DATA lpFD;
   ...
   if(IsChicago)
   {
      ...
      lpFD = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, sizeof(*lpFD));
      if(!lpFD) return(0xff);
