...
LPENV_VAR_ENTRY lpEnvVarEntry=NULL;
...
BOOL bBuildEnvVarEntry = FALSE;
   ...
   if(!lpEnvVarEntry)
   {
      ...
      lpEnvVarEntry = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, 0x4000);
...
LPENV_VAR_ENTRY lpE, lpE2;
   ...
   if(l2!=0)          /* non-zero length - new string has data attached */
   {
      ...
      {
         if(!(lpEnv = GlobalReAllocPtr(lpEnv, l3 + l4 + 16, ENV_REALLOC_FLAGS)))
         {
            return(TRUE);
         }
...
WORD new_env_size;
   ...
   if(lpMyEnv==(LPSTR)NULL)  /* first time it's been called... */
   {
      ...
      if(!(lpNewEnv = GlobalAllocPtr(ENV_MEM_FLAGS, env_size + 16)))
...
//         lpRval = GlobalAllocPtr(GMEM_MOVEABLE, lstrlen(lpStr) + 512);
         lpRval = GlobalAllocPtr(GMEM_MOVEABLE, _ihstrlen(lpStr) + 512);
...
//            w = lstrlen(lpRval) + lstrlen(lp4) + lstrlen(lp2); /* needed length */
      ...
      {
         lp3 = GlobalReAllocPtr(lpRval, w + 512, GMEM_MOVEABLE);
         if(!lp3)
         {
            PrintString(nomem_msg);
...
//      lpRval = GlobalAllocPtr(GMEM_MOVEABLE, lstrlen(lpStr) + 2);
      ...
      lpRval = GlobalAllocPtr(GMEM_MOVEABLE, wStrLen + 2);
...
static char digits[]="0123456789ABCDEF";
   ...
   if(l && toupper(lp1[l - 1])=='H')  /* a hex number, we assume... */
   {
      ...
      {
         lp2 = GlobalReAllocPtr(lp1, l + 2, GMEM_MOVEABLE | GMEM_ZEROINIT);
         if(!lp2)
         {
            GlobalFreePtr(lp1);
