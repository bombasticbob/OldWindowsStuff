...
char copy_string[256];  /* a place to put junk in the meantime  */
    ...
    p = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                       lstrlen(lp1) + 1);
...
{
   if(n==0) return(0);   // for 0 bytes, always equal!
...
static char function_buf[64];
    ...
    if(*op==OP_LEFTPAREN)                     /* FUNCTION CALL! */
    {
      ...
      fn = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
         ...
         if(w2==OP_RIGHTPAREN || w2==OP_COMMA)  // might be blank...
         {
            ...
            if(w>=op_start)         // it *is* only white-space (a blank arg)
            {
               ...
               *op = w2;                    // assigns current op
               ...
               if(lpArg[nArg] = GlobalAllocPtr(GMEM_MOVEABLE, 2))
               {
                  *(lpArg[nArg++]) = 0;
               }
               else
...
WORD w;
   ...
   lpRval = GlobalAllocPtr(GMEM_MOVEABLE, strlen(tbuf) + 2);
...
double FAR PASCAL CalcEvalNumber(LPSTR lpS)
{
LPSTR lp1, lp2;
WORD l, radix, count;
BOOL sign;
...
LPSTR FAR PASCAL CalcCopyString(LPSTR lpS, WORD wStart, WORD wLen)
{
LPSTR lpRval;
WORD w;
...
WORD FAR PASCAL CalcGetWindowInstTaskModule(LPCSTR szValue, WORD FAR *lpwType)
{
LPCSTR lp1 = szValue;
WORD w1, w2;
...
#define _PI_ 3.1415926535897932384626433832795
       ...
       LPSTR lpSaveParms = NULL;
         ...
         if(lp1)  // I FOUND IT! The function names match!
         {
            ...
            if(*lp1)  // we HAVE parameters...
            {
               ...
               lpSaveParms = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                            w3 + 16);
               ...
               else
               {
                  ...
                  w3 = (lpBI->dwEntryCount++);  // set up 'CALL' entry to mark
                  ...
                  *((HANDLE FAR *)lpBI->lpCodeBlock[w3].szInfo) =
                     SpawnThread(CalcUserFunctionThread, (HANDLE)w1,
   ...
   switch(fn)
   {
      ...
      case idCENTER:      // CENTER
         ...
         lpRval = GlobalAllocPtr(GMEM_MOVEABLE, w1 + 2);
         if(!lpRval) break;
      ...
      case idTASKLIST:  // TASKLIST    ([pattern])        - array of HTASK's
         ...
         lp2 = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, w1);
         if(!lp2) break;
         ...
         *lp2 = 0;
         ...
         do
         {
            ...
            *lp3 = 0;
            ...
            {
               w1 += 2048;
               lp3 = GlobalReAllocPtr(lp2, w1, GMEM_MOVEABLE | GMEM_ZEROINIT);
         ...
         lpRval = GlobalReAllocPtr(lp2, lstrlen(lp2) + 2, GMEM_MOVEABLE);
...
//         }
      ...
      case idGETPROFILESTRING: // GETPROFILESTRING([profile,] appname, valuename)
         ...
         {
            lp4 = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, 4096);
            if(!lp4)
            {
...
//            }
      ...
      case idFILESMATCH:    // FILESMATCH(string,string) - TRUE if binaries match
         ...
         lp3 = GlobalAllocPtr(GMEM_MOVEABLE, 32768);
         if(!lp3)
         {
            _lclose((HFILE)w1);
            _lclose((HFILE)w2);
      ...
      case idSTRING:  // STRING(count,ASCII code)
         ...
         {
            lpRval = GlobalAllocPtr(GMEM_MOVEABLE, w1 + 1);
      ...
      case idREGENUMKEYS:
         ...
         lpRval = GlobalAllocPtr(GHND, 0x10000L);
         if(!lpRval)
            break;
         ...
         lp2 = GlobalReAllocPtr(lpRval, (lp2 - lpRval) + 1,
                                GHND);
...
//         PrintErrorMessage(823);
      ...
      case idNETGETUNC:    // get 'UNC' name for specified name/device
         ...
         lp3 = GlobalAllocPtr(GHND, MAX_PATH);
         ...
         {
            lpRval = GlobalAllocPtr(GMEM_MOVEABLE, MAX_PATH);

            dw1 = MAX_PATH;

            if(lpWNetGetConnection(lp3, lpRval, &dw1)
               != WN_SUCCESS)
            {
               GlobalFreePtr(lpRval);
         ...
         else if(lp3[0] != '//' && lp3[1] != '//')
         {
            ...
            *lp2 = 0;  // initially, truncate 'lp3' to the device/drive only
            ...
            lpRval = GlobalAllocPtr(GMEM_MOVEABLE, MAX_PATH * 2);
      ...
      case idNETUSE:   // similar to 'Net Use'
         ...
         if(n == 0)
         {
          static const char CODE_BASED szDevices[][5]={
            ...
            lp3 = GlobalAllocPtr(GHND, MAX_PATH);
            lpRval = GlobalAllocPtr(GHND, 0xffff);
            ...
            for(w1 = 0; w1 < N_DIMENSIONS(szDevices); w1++)
            {
               ...
               UINT uiRval = lpWNetGetConnection(szDevices[w1], lp3, &cbSize);
...
//   }
   ...
   {
      lpRval = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, 1);
      if(lpRval) *lpRval = 0;
      else
      {
         PrintString(pNOTENOUGHMEMORY);
...
static char pARRAYSYNTAXERROR[]="?Syntax error in assignment array specification\r\n";
   ...
   {
      lpVal2 = GlobalAllocPtr(GHND, 2 * lstrlen(lpValue));
      ...
      *lp2 = 0;
      ...
      lp1 = GlobalReAllocPtr(lpVal2, (lp2 - lpVal2) + 1,
                             GMEM_MOVEABLE);
...
struct _enum_parms2_ {
 ...
 // 'wIDFlag' bit 0 = 'n>3'; bit 15 = child window
   ...
   if(!lpParms->lpRval)
   {
      lpParms->lpRval = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,0x8000);
...
DWORD cbSize;
   ...
   if(lpRegQueryValue(hKey, NULL, lpRval, &cbSize) != ERROR_SUCCESS)
   {
      GlobalFreePtr(lpRval);
   ...
   lp1 = GlobalReAllocPtr(lpRval, cbSize + 1, GHND);
...
NETRESOURCE rsc;
  ...
  else
  {
      ...
      rsc.lpRemoteName = (LPSTR)lpGetVDMPointer32W(szArg, 1);
...
WORD wAccess = 0, wFileMode;
   ...
   lpRval = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, sizeof(MYFILE));
...
{
   if(lpF->dwFilePos < _llseek(lpF->hFile, 0, SEEK_END))
   {
      _llseek(lpF->hFile, lpF->dwFilePos, SEEK_SET);
