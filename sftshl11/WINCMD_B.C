      ...
      if(_fstrcmp(lp1, lp2))     // that is, the evaluating did something
      {
       BOOL rval;
         ...
         while(*lp2)
         {
            ...
            lp3 = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
   ...
   *lp2 = 0;  /* terminates 1st arg pointed at by 'lp1' */
   ...
   else if(_fstrnicmp(lp1, "PATH=", 5)==0)
   {
      *lp2 = old_lp2;  // restore the character which *WAS* there
      ...
      return(SetCMDError(CMDPath(szPATH,lp1 + 4)));
   }
...
//                           wLen + 256);
      ...
      lp1 = MAKELP(LOWORD(GlobalDosAlloc(wLen + 256)), 0);
      ...
      {                              /* it's NOT a batch/REXX program file */
       ...
       DWORD dwProcessID, dwThreadID, dwhProcess;
         ...
         if(!dwLastProcessID || !hKernel32 || !hWOW32)  // I am supporting generic 32-bit thunks?
         {
            ...
            {
               (FARPROC)lpProc = MakeProcInstance(CMDRunProgramNotifyCallback,
                                                  hInst);
               if(lpProc)
               {
...
volatile int rval;
   ...
   if(!(h1=LocalAlloc(LMEM_MOVEABLE | LMEM_ZEROINIT, 2*QUAL_PATH_SIZE)) ||
      !(h2=LocalAlloc(LMEM_MOVEABLE | LMEM_ZEROINIT,
                      ...
                      + 2 * QUAL_EXT_SIZE )) ||
      !(path = (char _near *)LocalLock(h1)) || !(drive = LocalLock(h2)) ||
      !(lpPathVar = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                   lstrlen(lp1) + 1)))
   {
         ...
         if(lpRegOpenKey(hKey, NULL, &hKey) == ERROR_SUCCESS)  // optimization
         {
                         ...
                         == ERROR_SUCCESS; dw1++)
            {
               ...
               if(!_fstrnicmp(path, dir2, lstrlen(path)))
               {
                  ...
                  if(lpRegQueryValue(hKey, dir2, dir2,&dwLen)==ERROR_SUCCESS)
                  {
                     ...
                     dwLen = QUAL_PATH_SIZE;
                     if(lpRegQueryValue(hKey, dir2, dir2,&dwLen)
                        ==ERROR_SUCCESS)
                     {
   ...
   else
   {
      ...
      if((dw = GlobalSizePtr(lpBatchInfo->lpLabelList)) <=
         (lp1 - lpBatchInfo->lpLabelList + lstrlen(lpLabel)
           + 16 + sizeof(long)))
      {
         lp2 = GlobalReAllocPtr(lpBatchInfo->lpLabelList, dw + 4096,
         ...
         if(!lp2)
         {
          HCURSOR hOldCursor;
            ...
            if(!(lp2 = GlobalReAllocPtr(lpBatchInfo->lpLabelList, dw + 4096,
                                        GMEM_MOVEABLE | GMEM_ZEROINIT)))
            {
...
//            ;                           /* find next white-space char */
      ...
      else                    // STANDARD version of command!!
      {
         ...
         lpCommand = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                    2 * lstrlen(lp1) + 256);
   ...
   if(NotFlag)
   {
      ...
      lpTemp = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, MAX_PATH * 3);
   ...
   else
   {
      ...
      /* (file must also NOT be '.' or '..' at end of path...)        */
         ...
         else
         {
            ...
            plpDI[wNDirInfo] = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                              sizeof(DIR_INFO));
         ...
         if(w && w!=DIRINFO_ERROR)
         {
            plpPaths[wNDirInfo] = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT,
                                                 lstrlen(lp3) + 2);
...
BOOL rval;
   ...
   if(hWDBUTIL)
   {
      (FARPROC)wdbQueryErrorMessage  = GetProcAddress(hWDBUTIL, MAKELP(0,268));
      (FARPROC)wdbSQLProgram         = GetProcAddress(hWDBUTIL, MAKELP(0,269));

      (FARPROC)wdbCreateBrowseWindow = GetProcAddress(hWDBUTIL, MAKELP(0,234));
      (FARPROC)wdbOpenDatabase       = GetProcAddress(hWDBUTIL, MAKELP(0,203));
      (FARPROC)wdbCloseDatabase      = GetProcAddress(hWDBUTIL, MAKELP(0,204));
   ...
   lpCommand = GlobalAllocPtr(GMEM_MOVEABLE, 8192);
   ...
   *lpCommand = 0;
   ...
   do
   {
      ...
      if(wLen >= wSize)
      {
         lp1 = GlobalReAllocPtr(lpCommand, wLen + 1024, GMEM_MOVEABLE);
...
typedef struct tagODBC_ARRAY{
   ...
   char szConnect[1024 - sizeof(((struct tagODBC_ARRAY NEAR *)0)->lpBindInfo) -
                  (WORD)&(((struct tagODBC_ARRAY NEAR *)0)->lpBindInfo)];

   } ODBC_ARRAY, FAR *LPODBC_ARRAY;
...
long l1;
   ...
   if(retcode != SQL_SUCCESS)
   {
      if(retcode == SQL_SUCCESS_WITH_INFO)
      {
         ...
         lpSQLError(hEnv, SQL_NULL_HDBC, SQL_NULL_HSTMT, pSqlState, &l1,
                    lpBuf, cbBuf, &wLen);
   ...
   retcode = lpSQLDriverConnect(hDbc, hMainWnd, (LPSTR)lpCmd, lstrlen(lpCmd),
                                lpBuf, cbBuf, &w1,
                                SQL_DRIVER_COMPLETE);
   ...
   if(retcode != SQL_SUCCESS)
   {
      if(retcode == SQL_SUCCESS_WITH_INFO)
      {
         ...
         lpSQLError(hEnv, hDbc, SQL_NULL_HSTMT, pSqlState, &l1,
                    lpBuf, cbBuf, &wLen);
   ...
   retcode = lpSQLGetInfo(hDbc, SQL_FETCH_DIRECTION, &l1, sizeof(l1), &wLen);
   ...
   retcode = lpSQLGetInfo(hDbc, SQL_DRIVER_ODBC_VER, lpBuf, cbBuf, &wLen);
...
LPSTR lp1;
   ...
   do
   {
      retcode = lpSQLNumResultCols(lpOA->hStmt, &w2);
         ...
         if(*lp2)  // there is something else on the line...
         {
            ...
            if(w1 < wNODBCX)
            {
               ...
               lpSQLError(lpODBCX[w1].hEnv, lpODBCX[w1].hDbc, lpODBCX[w1].hStmt,
                          pSqlState, &l1, tbuf, sizeof(tbuf), &wLen);
...
//            }
      ...
      case CMDODBC_CURSOR:   // assign the cursor name
         ...
         retcode = lpSQLSetCursorName(lpODBCX[w1].hStmt, lp2, SQL_NTS);
         if(retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
         {
            rval = TRUE;
            break;
      ...
      case CMDODBC_OPEN:
         ...
         do
         {
            retcode = lpSQLExecDirect(lpODBCX[w1].hStmt, tbuf,
                                      lstrlen(tbuf));
      ...
      case CMDODBC_SQL:
         ...
         if(*lp2)     // SQL command is on remainder of command line...
         {
            do
            {
               retcode = lpSQLPrepare(lpODBCX[w1].hStmt, lp2, lstrlen(lp2));
         ...
         *lpCommand = 0;
         ...
         else
         {
            do
            {
               retcode = lpSQLPrepare(lpODBCX[w1].hStmt, lp1, lstrlen(lp1));
         ...
         if(KEYWORD_MATCH(lp2,szBEGIN,wLen))
         {
            ...
            if(retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
            {
               ...
               lpSQLError(lpODBCX[w1].hEnv, lpODBCX[w1].hDbc,
                          lpODBCX[w1].hStmt, pSqlState, &l1,
                          tbuf, sizeof(tbuf), &wLen);
...
//               break;
               ...
               lpSQL = GlobalAllocPtr(GPTR, 0xffff);
               if(!lpSQL)
               {
                  PrintString(pNOTENOUGHMEMORY);
                  ...
                  while(*lp1)
                  {
                   BOOL bChanged = FALSE;
                     ...
                     *tbuf2 = 0;
                     ...
                     lpSQLDescribeCol(lpODBCX[w1].hStmt, w2, tbuf2, sizeof(tbuf2),
                                      &s1, &s2, &dw1, &s3, &s4);
               ...
               if(!rval && *lpSQL)
               {
                HSTMT hNewStmt;
                     ...
                     retcode == SQL_SUCCESS_WITH_INFO)
                  {
                     do
                     {
                        retcode = lpSQLExecDirect(hNewStmt, lpSQL, SQL_NTS);
...
RETCODE retcode;
   ...
   lpRval = GlobalAllocPtr(GHND, 0x8000);
   if(!lpRval) return(NULL);       // no memory...


   if(!wColumn)        // retrieve ALL un-bound columns
   {
      retcode = lpSQLNumResultCols(lpODBCX[w1].hStmt, &wNCol);
...
//         lpRval[dw1] = 0;
   ...
   if(lpRval)
   {
      lp1 = GlobalReAllocPtr(lpRval, lstrlen(lpRval) + 1,
                             GMEM_MOVEABLE);
...
static const BYTE pTP[]={  // 0 for CHAR, 1 for DWORD
   ...
   retcode = lpSQLNumResultCols(lpODBCX[w0].hStmt, &wNCol);
   if(retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO)
   {
      GlobalFreePtr(lpRval);
      return(NULL);
...
char tbuf[256];
   ...
   retcode = lpSQLGetCursorName(lpODBCX[w1].hStmt, tbuf, sizeof(tbuf),
                                &cbCursor);
   ...
   lpRval = GlobalAllocPtr(GPTR, lstrlen(tbuf) + 1);
...
LPSTR lp1, lpRval;
   ...
   if(!lpRval)
   {
      lpRval = GlobalAllocPtr(GHND, lstrlen(tbuf) + 1);
...
RETCODE retcode;
   ...
   lpRval = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, 0x8000);
   ...
   retcode = lpSQLTables(hStmt, "", 0, "", 0, "", 0, "", 0);
   ...
   while((retcode = lpSQLNumResultCols(hStmt, &w1)) == SQL_STILL_EXECUTING)
   {
      MthreadSleep(50);
   ...
   *lpRval = 0;
   ...
   lp1 = GlobalReAllocPtr(lpRval, lstrlen(lpRval) + 1, GMEM_MOVEABLE);
   ...
   *lpRval = 0;
   ...
   while(TRUE)
   {
      retcode = lpSQLDrivers(hEnv,
                             ...
                             tbuf1, sizeof(tbuf1), &wLen1,
                             tbuf2, sizeof(tbuf2) - 1, &wLen2);
