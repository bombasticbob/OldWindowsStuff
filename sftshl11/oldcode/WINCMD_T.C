...
LPSTR lpTemp;
   ...
   else
   {
      if(MthreadGetCurrentThread() != hBatchThread)
      {
         ...
         SetEnvString("RETVAL","");  // blank value for 'RETVAL'
...
extern HWND hdlgQueue;  // declared in wincmd.c
   ...
   if(TotalCopiesSubmitted)
   {
      ...
      {
         wsprintf(work_buf, "%ld file sets", TotalCopiesSubmitted);
      }
      else
      {
         lstrcpy(work_buf, "1 file set");
      }

      lstrcat(work_buf," added to COPY QUEUE for background copy\r\n\n");
...
static char pCTRLZ[]="\x1a";
      ...
      || lpCopyInfo->next_source)
   {
      ...
      do
      {
         ...
         if(lpCopyInfo->hSrc==HFILE_ERROR)
         {
            lstrcpy(work_buf, "?Error opening file: \"");
            lstrcat(work_buf, lp1);
            lstrcat(work_buf, "\"!!");

            ierr = MyErrorMessageBox(hMainWnd, work_buf, pCOPYERROR);
            if(ierr!=IDRETRY)
            {
               if(lpCopyInfo->hDest!=HFILE_ERROR)
               {
         ...
         memset(work_buf, 0, sizeof(work_buf)); /* initially zero buffer */

         if(lpDirInfo)         /* we're not appending files together */
         {                                    /* make copy of pattern */
            _fstrncpy(work_buf, lpDirInfo->fullname,
                      sizeof(lpDirInfo->fullname));

            ShortToLongName(work_buf, lpCopyInfo->srcpath);
         }
         else                /* using the '+' on multiple input files */
         {
            lstrcpy(work_buf, lpCopyInfo->dest);
         }

         if(!lpDirInfo ||
            !UpdateFileName(work_buf, lpCopyInfo->dest, work_buf))
         {
            if(!lpDirInfo) DosQualifyPath(work_buf, work_buf);
                                 // this does 'pre-crunching' on device names


            if((hTemp = MyOpenFile(work_buf, &(lpCopyInfo->ofdest),
                                OF_READ | OF_SHARE_DENY_NONE))!= HFILE_ERROR
            || (hTemp = OpenFile(work_buf, &(lpCopyInfo->ofdest),
                                OF_READ | OF_SHARE_DENY_NONE))!= HFILE_ERROR)
            {
         ...
         /* if the file exists, and is the same as input file! */
            ...
            if(DestIsCON)
            {
               lpCopyInfo->hDest = OpenFile(work_buf,
                                            &(lpCopyInfo->ofdest),
                                            OF_READWRITE);
            }
            else if(IsValidPortName(work_buf))
            {
               lpCopyInfo->hDest = (HFILE)CreatePrinterDCFromPort(work_buf);
            ...
            else
            {
               ...
               if(IsChicago && !bPlusFound) // not concatenating files either
               {
...
#ifdef WIN32
                     GetDriveType(work_buf) == DRIVE_REMOVABLE)
#else  // WIN32
                     GetDriveType(toupper(*work_buf) - 'A') == DRIVE_REMOVABLE)
#endif // WIN32
                  {
                     bSpecialWriteMode = TRUE;

                     lpCopyInfo->ofdest.nErrCode =
                        MyCreateFile(work_buf,
                     ...
                     {
                        lpCopyInfo->ofdest.nErrCode =
                           GetShortName(lpCopyInfo->ofdest.szPathName, work_buf);
                     }
                  ...
                  {
                     lpCopyInfo->hDest = MyOpenFile(work_buf,
                                            &(lpCopyInfo->ofdest),
