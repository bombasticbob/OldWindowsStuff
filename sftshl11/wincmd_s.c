      ...
      if(!redir_flag)
      {
         ...
         if(lpParseInfo->sbpPipeNext)
         {
          LPSTR lp1, lp2, lp3;
            ...
            if(*lpParseInfo->sbpPipeNext)
            {
               lp1 = GlobalAllocPtr(GMEM_MOVEABLE,
...
char __based(lpScreen) *pScreen, __based(lpScreen) *pAttrib;
   ...
   else
   {
      ...
      if(redirect_output == HFILE_USE_DC)
      {
         ...
         lp1 = GlobalAllocPtr(GPTR, ctr + sizeof(WORD) + 1);
         if(!lp1)
         {
            return(TRUE); // error
         }
   ...
   if(CheckScreenScroll(FALSE)) return(TRUE);
   ...
   pScreen = &(screen[curline][curcol]);
   pAttrib = &(attrib[curline][curcol]);
...
//            // end temporary section
      ...
      else switch(chr)
      {
         case 0x0d:   /* CR */
            ...
            {
               *(pAttrib++) = (char)cur_attr;  /* extend current attribute! */
            }
         ...
         case 0x09:   /* TAB */
            ...
            while(i < curcol)     // fill in characters I skipped over...
            {
               if(i >= SCREEN_COLS)
               {
                  ...
                  pScreen = &(screen[curline][i]);
                  pAttrib = &(attrib[curline][i]);
               }
         ...
         case 0xff:   /* DEL */
            ...
            {
               _hmemcpy((void FAR *)&(screen[curline][curcol]),
                        (void FAR *)&(screen[curline][curcol + 1]),
                        ((end_line - curline) * SCREEN_COLS
                         + end_col - curcol - 1) * sizeof(screen[0][0]));

               _hmemcpy((void FAR *)&(attrib[curline][curcol]),
                        (void FAR *)&(attrib[curline][curcol + 1]),
                        ((end_line - curline) * SCREEN_COLS
                         + end_col - curcol - 1) * sizeof(attrib[0][0]));
            }
      ...
      switch(chr)
      {
         ...
         case 'K':  /* erase to EOL */
            ...
            else
            {
               ...
               {
                  screen[curline][i] = 0;
                  attrib[curline][i] = (char)cur_attr;
               }
...
int actual_text_height, actual_text_width, vpos, hpos,
   ...
   {
      _hmemcpy((void FAR *)screen,
               (void FAR *)(((char FAR *)screen) + SCREEN_COLS),
               (SCREEN_LINES - 1) * sizeof(screen[0][0]) * SCREEN_COLS);

      _hmemcpy((void FAR *)attrib,
               (void FAR *)(((char FAR *)attrib) + SCREEN_COLS),
               (SCREEN_LINES - 1) * sizeof(attrib[0][0]) * SCREEN_COLS);

      _hmemset((void FAR *)(screen[SCREEN_LINES - 1]), 0,
               sizeof(screen[0][0]) * SCREEN_COLS);

      _hmemset((void FAR *)(attrib[SCREEN_LINES - 1]), cur_attr,
               sizeof(attrib[0][0]) * SCREEN_COLS);
   ...
   else if(curline<0)
   {
      ...
      _hmemset((void FAR *)screen, 0,
               sizeof(screen[0][0]) * SCREEN_COLS);
      _hmemset((void FAR *)attrib, cur_attr,
               sizeof(attrib[0][0]) * SCREEN_COLS);
...
static char pFatal[]="?Unable to load MESSAGE string - forcing CTRL-BREAK!\r\n\n";
   ...
   lpString = GlobalAllocPtr(GMEM_MOVEABLE, 256); // 256 bytes for error buf
...
#define TEMP_BUF_SIZE 4096  /* size big enough for just about any prompt */
   ...
   lpTempBuf = GlobalAllocPtr(GMEM_MOVEABLE | GMEM_ZEROINIT, TEMP_BUF_SIZE);
   if(!lpTempBuf)
   {
...
{
   _hmemset((void FAR *)screen, 0,
            sizeof(screen[0][0]) * SCREEN_LINES * SCREEN_COLS);
                                                  /* zero xlates as space */

   _hmemset((void FAR *)attrib, cur_attr,
            sizeof(attrib[0][0]) * SCREEN_LINES * SCREEN_COLS);
